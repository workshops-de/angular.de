...
async function fetchGravatars() {
  ...
  console.log(
    `📥 Downloading gravatars...${forceDownload ? " (force refresh)" : ""}`
  );
  ...
  for (const file of userFiles) {
    ...
    const gravatarMatch = content.match(
      /gravatar_uid:\s*["']?([a-f0-9]+)["']?/i
    );
    const isTeam =
      content.includes("team: true") || content.includes("team:true");
    const isTrainer =
      content.includes("trainer: true") || content.includes("trainer:true");
    ...
    if (gravatarMatch) {
      ...
      for (const size of sizes) {
        ...
        if (result.status === "downloaded") {
          ...
          errors++;
          console.error(
            `  ✗ ${gravatarUid}-${size}x${size}.jpg: ${result.error}`
          );
        }
      }
    }
  ...
  console.log(
    `✅ Downloading gravatars...done (${downloaded} downloaded, ${skipped} skipped, ${errors} errors)\n`
  );
}
