...
description: "Nutzt alle Features von ES2015 und eine strikte Typisierung in eurer AngularJS Anwendung. Microsofts JavaScript Superset TypeScript bietet euch all das!"
author: "Sascha Brink"
...
angular.module("myApp", ["ngRoute", routes, navigation, booksIndex, booksShow]);
```
...
import booksApiModule, {
  IBook,
  IBooksApi,
} from "../../services/books-api/books-api";

class BooksIndex {
  books: IBook[];

  constructor(booksApi: IBooksApi) {
    booksApi.all().then((books) => (this.books = books));
  }
}
...
angular.module(moduleName, [booksApiModule]).component("booksIndex", {
...
```typescript
angular.module(moduleName, []).service("booksApi", HttpBooksApi);
```
