...
```javascript
angular.module("angularjsDE", []).factory("restService", function ($http) {
  return {
...
```javascript
it("should call $http.get in auth", inject(function (
  restService,
  $httpBackend
) {
...
Folgende Objekte sind in dem Modul _ngMock_ derzeitig implementiert.
